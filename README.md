...
The prototype includes deterministic seeds, a replayable three-step route coach, eight calibration moves, optional eight-move challenges, twelve-move journey soft stops, four feedback tiers, local save and restore with a v1 to v2 migration path, exportable session logs, keyboard controls, reduced motion, silent-by-default audio, and low-end device fallbacks. The coach is decorative, can be skipped or replayed, hides a stale route illustration behind a clear safety note, and keeps the first-screen input path independent. Closing or skipping it leaves a brief noninteractive "Your turn" cue. During an active board drag, only the board surface suppresses page movement, so the surrounding page remains scrollable. Status and teaching text reserve mobile space to reduce layout jumps. After every twelve valid free-play moves, a soft settlement card lets the player continue or save without changing the board or rules. After saving, the card offers Close for now and Play another journey. A recursive V0.2 resource budget check keeps the local resource graph bounded.
