...
const cloneCell = ({ row, column }) => ({ row, column });
const cloneCells = (cells = []) => cells.map(cloneCell);
const cellKey = ({ row, column }) => `${row},${column}`;

export function createPresentationModel({ state, path = [], action = null, result = null }) {
  const authoritativePath = result?.valid === true ? cloneCells(result.path || []) : [];
  const replacements = result?.valid === true
    ? (result.replacements || []).map(({ row, column, node }) => ({ row, column, node }))
    : [];
...
export function createPresenter({
  ...
  configureCell = (button) => { button.tabIndex = 0; },
  focusCellButton = (button) => button?.focus({ preventScroll: true }),
  ...
  function centerOf({ row, column }, width, height) {
    return {
  ...
  function renderPath(model) {
    ...
    if (model.traceFeedback === "backtrack" || model.traceFeedback === "truncate") {
      ui.pathLayer.append(svgElement("path", {
        class: "backtrack-hook",
        "data-action": model.traceFeedback,
        d: `M${endpoint.x + 7} ${endpoint.y - 13} C${endpoint.x + 20} ${endpoint.y - 13} ${endpoint.x + 20} ${endpoint.y + 7} ${endpoint.x + 6} ${endpoint.y + 7} M${endpoint.x + 6} ${endpoint.y + 7} l6 -6 M${endpoint.x + 6} ${endpoint.y + 7} l7 5`,
        "aria-hidden": "true"
      }));
    }
  ...
  function render({ state, path = [], message = null, action = null, result = null, focusCell = null }) {
    clearTransient("render");
    const model = createPresentationModel({ state, path, action, result });
    renderBoard(state, model, focusCell);
    ui.cat.textContent = `Cat ${state.currentCat}`;
    ui.growth.textContent = String(state.growth);
    ui.hands.textContent = String(state.validHands);
    ...
    if (mission.enabled) {
      ui.mission.textContent = mission.active
        ? `Rendezvous challenge: ${mission.hands} / 8 moves, ${mission.meetings} / 3 meetings`
        : mission.result === "complete" ? "Challenge complete. Keepsake unlocked." : "Challenge ended. All journey points are kept.";
    }
  ...
  function applyPreferences({ reducedMotion: nextReducedMotion }) {
    reducedMotion = Boolean(nextReducedMotion);
    body.classList.toggle("reduce-motion", reducedMotion);
    body.classList.toggle("low-end", lowEnd);
  }

  return Object.freeze({ render, clearTransient, applyPreferences, lowEnd });
}
